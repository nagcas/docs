...
const url = new URL('https://api.terraquakeapi.com/v1/stations');
...
url = 'https://api.terraquakeapi.com/v1/stations/code'
...
const url = `https://api.terraquakeapi.com/v1/stations/code?code=${stationCode}`;
...
<Info>
  Station code lookup is **case-insensitive**. Both `ACATE` and `acate` will
...
```bash cURL
curl "https://api.terraquakeapi.com/v1/stations/geojson?limit=500"
```
...
url = 'https://api.terraquakeapi.com/v1/stations/geojson'
params = {'limit': 500}
...
map.addLayer({
  ...
  paint: {
    'circle-radius': 6,
    'circle-color': ['match', ['get', 'status'], 'open', '#10b981', '#6b7280'],
  },
});
```
...
```bash cURL
curl "https://api.terraquakeapi.com/v1/stations/status/open"
```
...
```javascript JavaScript
const response = await fetch(
  'https://api.terraquakeapi.com/v1/stations/status/open',
);
const data = await response.json();
...
```bash cURL
curl "https://api.terraquakeapi.com/v1/stations/status/closed"
```
...
console.log(
  `Active: ${stationsOpen} (${((stationsOpen / totalStations) * 100).toFixed(1)}%)`,
);
...
class NetworkMonitor:
    def __init__(self):
        self.base_url = 'https://api.terraquakeapi.com/v1'
...
<Card
  title='Cache Station Data'
  icon='database'
>
  Station data changes infrequently. Cache responses for 24 hours: ```python
...
<Card
  ...
  icon='filter'
>
  Use status endpoints (`/status/open`) when you only need active stations to
  reduce payload size.
</Card>
