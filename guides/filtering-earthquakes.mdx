...
<Tabs>
  <Tab title="Today's Events">
    ...
    response = requests.get('https://api.terraquakeapi.com/v1/earthquakes/today')
    earthquakes = response.json()
    print(f"Found {earthquakes['totalEarthquakes']} events today")
    ```

    ```javascript JavaScript
    const response = await fetch('https://api.terraquakeapi.com/v1/earthquakes/today');
  ...
  <Tab title="Recent Events">
    ...
    ```bash cURL
    curl https://api.terraquakeapi.com/v1/earthquakes/recent
    ```

    ```python Python
    import requests

    response = requests.get('https://api.terraquakeapi.com/v1/earthquakes/recent')
    data = response.json()
    ...
    ```javascript JavaScript
    const response = await fetch('https://api.terraquakeapi.com/v1/earthquakes/recent');
    const data = await response.json();
  ...
  <Tab title="Last Week">
    ...
    ```bash cURL
    curl https://api.terraquakeapi.com/v1/earthquakes/last-week
    ```

    ```python Python
    import requests

    url = 'https://api.terraquakeapi.com/v1/earthquakes/last-week'
  ...
  <Tab title="Specific Month">
    ...
    ```bash cURL
    curl "https://api.terraquakeapi.com/v1/earthquakes/month?year=2024&month=3"
    ```

    ```python Python
    import requests

    url = 'https://api.terraquakeapi.com/v1/earthquakes/month'
    params = {
        'year': 2024,
        'month': 3,
        'limit': 50
    ...
    ```javascript JavaScript
    const url = new URL('https://api.terraquakeapi.com/v1/earthquakes/month');
  ...
  <Tab title="Date Range">
    ...
    ```bash cURL
    curl "https://api.terraquakeapi.com/v1/earthquakes/range?startdate=2024-01-01&enddate=2024-03-31"
    ```

    ```python Python
    import requests

    url = 'https://api.terraquakeapi.com/v1/earthquakes/range'
    params = {
...
```javascript JavaScript
const url = new URL('https://api.terraquakeapi.com/v1/earthquakes/magnitude');
...
<Info>
  The `mag` parameter represents the **minimum** magnitude threshold. Only
  earthquakes **greater than** this value will be returned.
</Info>
...
url = 'https://api.terraquakeapi.com/v1/earthquakes/depth'
params = {
    'depth': 10,
    'limit': 50,
    'sort': '-depth'
...
const response = await fetch(
  'https://api.terraquakeapi.com/v1/earthquakes/depth?depth=0&sort=depth',
);
const data = await response.json();
...
<Note>
  See the [Location Queries](/guides/location-queries) guide for the complete
  list of supported Italian regions.
</Note>
...
<Steps>
  ...
  <Step title="Build the query">
    ...
    ```javascript JavaScript
    const url = new URL('https://api.terraquakeapi.com/v1/earthquakes/range');
...
>
  Sort by relevant fields to get the most important data first: ```bash
  ?sort=-magnitude # Largest first ?sort=-time # Most recent first ```
</Card>
...
<Card
  ...
  icon='filter'
>
  Request only the fields you need: ```bash ?fields=time,magnitude,place ```
</Card>
