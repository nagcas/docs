...
<Info>
  Default limit is 50. Use `limit=100` for fewer requests, or `limit=25` for
...
response = requests.get(
    'https://api.terraquakeapi.com/v1/earthquakes/range',
...
data = client.get_earthquakes('earthquakes/v1/recent', {'limit': 100}, cache_ttl=3600)
...
<Warning>
  While the TerraQuake API doesn't currently enforce strict rate limits,
  implement rate limiting to be a good API citizen and prepare for future
  restrictions.
</Warning>
...
for i in range(100):
    response = client.get('https://api.terraquakeapi.com/v1/earthquakes/today')
...
class TerraQuakeClient:
    def __init__(self, base_url='https://api.terraquakeapi.com/v1'):
