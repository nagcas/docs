...
<img
  ...
  src='/images/terraquake-icon.png'
...
```bash cURL
curl https://api.terraquakeapi.com/v1/earthquakes/today?limit=10
```

```javascript JavaScript
const response = await fetch(
  'https://api.terraquakeapi.com/v1/earthquakes/today?limit=10',
);
...
<Tip>
  Most endpoints are public and don't require authentication. Get started
  immediately!
</Tip>
...
<Note>
  The API processes and enhances INGV data with features like pagination,
  flexible filtering, and consistent response formatting to make integration
  easier.
</Note>
