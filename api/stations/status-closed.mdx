---
title: 'GET v1/stations/status/closed'
...
```bash cURL
curl "https://api.terraquakeapi.com/v1/stations/status/closed?limit=100&page=1"
```

```javascript JavaScript
const response = await fetch(
  'https://api.terraquakeapi.com/v1/stations/status/closed?limit=100',
);
...
url = "https://api.terraquakeapi.com/v1/stations/status/closed"
...
```php PHP
<?php
$url = "https://api.terraquakeapi.com/v1/stations/status/closed?limit=100";
...
```json
{
  ...
  "meta": {
    "method": "GET",
    "path": "/v1/stations/status/closed?limit=2&page=1",
...
async function analyzeNetworkEvolution() {
  ...
  const closedResp = await fetch(
    'https://api.terraquakeapi.com/v1/stations/status/closed?limit=500',
  );
  ...
  console.log(
    `Total lifetime stations: ${openData.totalStationsOpen + closedData.totalStationsClosed}`,
  );

  const retentionRate = (
    (openData.totalStationsOpen /
      (openData.totalStationsOpen + closedData.totalStationsClosed)) *
    100
  ).toFixed(1);
